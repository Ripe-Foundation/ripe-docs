...
**Step 3.** To take collateral out, use the **-** control on the asset's row in the collateral table (the **+** deposits, the **-** withdraws). Ordinary withdrawal is unavailable while the account is in liquidation or valuation quarantine. Repay far enough below the current borrowing limit to satisfy the withdrawal buffer; a full repayment removes the debt-health constraint, while the normal vault, token, pause, and account controls still apply.
