...
The button reads **Approve** whenever the Teller lacks enough allowance for the token you're paying with. Approve it in your wallet and the button becomes Payback. A later payment may need another approval depending on the remaining allowance, token behavior, and token state. Your debt ratio improves as soon as the payment lands.

**Step 3.** To take collateral out, use the **-** control on the asset's row in the collateral table (the **+** deposits, the **-** withdraws). No ordinary withdrawal is available while the account is `inLiquidation`. When debt is above current max-borrow capacity, whole-account housekeeping also reverts an ordinary withdrawal, including one of zero-LTV collateral. At exactly the maximum, collateral that contributes borrowing power has no withdrawal room; zero-LTV collateral can still be withdrawable if the other checks pass. Repay until debt is at or below the current maximum to clear the liquidation restriction, and far enough below it to satisfy the withdrawal safety buffer for borrowing-power collateral. Any withdrawal remains subject to vault custody and the ordinary token, pause, quarantine, and action-block checks. A true full repayment removes the debt-health constraint, but those other controls still apply.
